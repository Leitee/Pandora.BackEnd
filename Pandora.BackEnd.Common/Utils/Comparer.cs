...
namespace Pandora.BackEnd.Common.Utils
{
    public static class Comparer
    {
        ...
        public static bool AreObjectsEqual(object objectA, object objectB, ref IList<PropertyInfo> misMatchProperties, params string[] ignoreList)
        {
            bool result;
                ...
                foreach (PropertyInfo propertyInfo in objectType.GetProperties(BindingFlags.Public | BindingFlags.Instance).Where(p => p.CanRead && !ignoreList.Contains(p.Name)))
                {
                    ...
                    if (CanDirectlyCompare(propertyInfo.PropertyType))
                    {
                        if (!AreValuesEqual(valueA, valueB))
                        {
                            ...
                            misMatchProperties.Add(propertyInfo);
                            result = false;
                        }
                    }
                        ...
                        else if (valueA != null && valueB != null)
                        {
                            ...
                            else
                            {
                                for (int i = 0; i < collectionItemsCount1; i++)
                                {
                                    ...
                                    else if (!AreObjectsEqual(collectionItem1, collectionItem2, ref misMatchProperties, ignoreList))
                                    {
                                        Console.WriteLine("Item {0} in property collection '{1}.{2}' does not match.", i, objectType.FullName, propertyInfo.Name);
                                        misMatchProperties.Add(propertyInfo);
                                        result = false;
                    ...
                    {
                        if (!AreObjectsEqual(propertyInfo.GetValue(objectA, null), propertyInfo.GetValue(objectB, null), ref misMatchProperties, ignoreList))
                        {
                            Console.WriteLine("Mismatch with property '{0}.{1}' found.", objectType.FullName, propertyInfo.Name);
                            misMatchProperties.Add(propertyInfo);
                            result = false;
